...
setEnergy((prevEnergy) => {
  ...
  if (newEnergy <= 0) {
    ...
    clearTimeRemaining(); // Ensure previous timer is cleared

    const timer = setInterval(() => {
      const newTimeLeft = new Date(endTime) - new Date();
      if (newTimeLeft <= 0) {
        clearInterval(timer);
        clearTimeRemaining(); // Clear the timer when done
        setEnergy(battery.energy); // Reset energy or handle as needed
      } else {
...
if (points === 150) {
      ...
      if (newTimeLeft > 0) {
        ...
        const timer = setInterval(() => {
          ...
          if (updatedTimeLeft <= 0) {
            clearInterval(timer);
            clearTimeRemaining(); // Use the function here
            setEnergy(battery.energy);
          } else {
  ...
  const handleCharacterSelect = async (character) => {
    ...
    // Ensure that `selectedCharacterOne` has the correct value
    console.log('Selected character state before Firestore update:', character);
    ...
    if (id) {
      try {
        ...
        console.log('Character updated in Firestore successfully');
      } catch (error) {
  ...
  const handleSaveFullName = async (newBalance) => {
    ...
    console.log('Starting handleSaveFullName with:', { fullName, newBalance, selectedCharacterOne });
  ...
  useEffect(() => {
    console.log('Selected character updated:', selectedCharacterOne);
      ...
      <h1 className='flex w-full justify-center items-center space-x-1 pb-2'>
        <img src='/loader.webp'className='w-[30px]' alt='engagecoin'/>
         <span className='text-[#fff] text-[32px] font-bold'>
           <span className='pl-[2px]'>{formatNumber(balance + refBonus)} <span className='text-btn4'>KAPI</span></span>
        </span>
      </h1>
      <div>
  ...
  <div class="after"></div>
      ...
      {energy === 0 && points === 0 ? (
          ...
          <span className='text-[#fff] font-semibold text-[24px]'>
            <span className='pl-[2px]'>{points} <span className='text-btn4'>KAPI</span></span>
          </span>
          <button 
            onClick={handleClaim} 
            disabled={points === 0} 
        ...
        <div className={`${
          ...
          <div className="w-full flex justify-center flex-col items-center space-y-3">
            ...
            <h3 className="font-medium text-[24px] text-[#ffffff] pt-2 pb-2">
              <span className='text-accent'>+{points}</span> KAPI
            </h3>
            <p className="pb-6 text-[#bfbfbf] text-[15px] w-full text-center">
              Keep grinding! something huge is coming! Get more KAPI ow! 
            </p>
  ...
  <>
    ...
    <div className='w-full flex items-center justify-center space-x-5'>
      {characters.map((character) => (
        <div key={character.name} className='w-[110px] h-[110px] relative flex items-center justify-center'>
          <div
    ...
    <div className="w-full flex flex-col items-center space-y-3 px-6">
      ...
      <div className='w-full px-6'>
        <button
          ...
          onClick={() => handleSaveFullName(balance + 100)}
        >
